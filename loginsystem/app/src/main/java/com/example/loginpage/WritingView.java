...
public class WritingView extends View {
    ...
    // New attribute to handle the loaded image
    ...
    // New method to load an image onto the canvas
    public void loadImage(Bitmap image) {
        Log.d("ImageLoad", "loadImage called with image: " + image);

        if (image!=null){
            loadedImage = image;
            invalidate();
        }else {
            Log.e("ImageLoad", "Loaded image is null");
        }
    }
    ...
    protected void onDraw(@NonNull Canvas canvas) {
        ...
        int backgroundColor = canvasBackgroundColor;
        canvas.drawColor(backgroundColor);

        // Draw the loaded image
        if (loadedImage != null) {
            Log.d("ImageLoad", "onDraw called");
            canvas.drawBitmap(loadedImage, 0, 0, mBitmapPaint);
        }
